    ...
    def step(self, time_step, is_evaluation=False):
        ...
        if self._prev_info_state and not is_evaluation:
            ...
            if time_step.last():  # prepare for the next episode.
                ...
                for s,a in self.episode_states:
                    sa = (s, a)
                ...
                self.episode_states.append((self._prev_info_state,self._prev_action))
...
class LUGLBLightGBM(DCLF):
    ...
    def get_model_qs(self, state_features, legal_actions):
        ...
        #print(legal_actions)
        q_values = np.array(legal_actions)
        #q_values = [0] * len(legal_actions)



        feature_actions = [[1] + list(state_features)]
        #print(feature_actions)
        top = self.model.predict(feature_actions)[0]
        #print(q_values, top, "sdfafasfs")

        q_values[q_values!=top] = 0
        q_values[q_values == top] = 1
    ...
    def train_supervised(self):
        ...
        for (state, action), Q, in self._tbr.items():
            #for action, Q in enumerate(self._q_values[state]):
                print(Q)
                all_features.append(list([np.mean(Q)]) + list(state))
                all_Qs.append(action)
        ...
        clf = LGBMClassifier(n_jobs=6, n_estimators=n_estimators_, num_leaves=200,
                            linear_tree=True,verbose = -100)
        clf.fit(X, y,verbose =False)
        self.model = clf
        mse = metrics.accuracy_score(y, self.model.predict(X))
        #r2 = metrics.auc(y, self.model.predict(X))
