    ...
    def step(self, time_step, is_evaluation=False):
        ...
        if self._prev_info_state and not is_evaluation:
            ...
            if (sa not in self._tbr):
                self._tbr[sa] = []
            v = self._tbr[sa]
            v.append([target, self._prev_info_next_state, self._prev_info_next_state_next])
        ...
        if not is_evaluation:
            ...
            obs = np.array(child_state.observation_tensor(self._player_id))


            c_obss = []
            c_obss.extend(obs)
            N = len(obs) * 10
            for a in child_state.legal_actions():
                grand_child = child_state.clone()
                grand_child.apply_action(a)
                c_obs = np.array(grand_child.observation_tensor(self._player_id))
                #c_obs = ((obs != c_obs) * 1)
                c_obss.extend(c_obs)

            c_obss+= [ 0 ] * (N - len(c_obss))



            self._prev_info_next_state = tuple(obs)
            self._prev_info_next_state_next = tuple(c_obss)
...
class LUGLBLightGBM(DCLF):
    ...
    def get_model_qs(self, state_features, legal_actions):
        ...
        for i, action in enumerate(legal_actions):
            ...
            #     grand_child.apply_action(a)
            #     c_obs = np.array(
            #         grand_child.observation_tensor(self._player_id))
            #     c_obs = ((obs != c_obs) * 1)
            #     c_obss.extend(c_obs)
            #
            # c_obss += [0] * (N - len(c_obss))
            #
            actions.append(c_obss)

        actions = self.transformer(np.array(actions))
    ...
    def train_supervised(self):
        ...
        for (state, action), q_fstates, in self._tbr.items():
            #for action, Q in enumerate(self._q_values[state]):
                ...
                all_next = q_fstates[0][2]
                all_Qs.append(np.mean(Q))
                all_nexts.append(all_next)

        X = np.array(all_features)
        y = np.array(all_Qs)
        all_nexts = np.array(all_nexts)
        ...
        input = keras.Input(shape=(X.shape[1],), name="input")
        ...
        # x = Dense(100, activation="tanh")(x)
        # x = keras.layers.BatchNormalization()(x)
        transformer = keras.Model([input], x)
        d = Dense(all_nexts.shape[1], activation="linear")(x)
        #d2 = Dense(1, activation="linear")(x)


        print(X.shape, "transformed")

        transformer_full = keras.Model([input], [ d])

        transformer_full.compile(loss="mse", optimizer="adam")
        ...
        X_train, X_test, y_train, y_test, all_nexts_train, all_nexts_test = train_test_split(
            X, y, all_nexts, test_size=0.10 )

        print(transformer_full.summary())
        transformer_full.fit(X_train, [ all_nexts_train],
                             validation_data=(X_test, [all_nexts_test]),
                             verbose=True, epochs = 50)


        X_train = transformer.predict(X_train)
        X_test = transformer.predict(X_test)
        ...
        #         emb_train_evo = model.predict(X_train, verbose=False)
        #         emb_test_evo = model.predict(X_test, verbose=False)
        #
        #         clf.fit(emb_train_evo,y_train )
        #
        #         y_hat_test_evo = clf.predict(emb_test_evo)
        #         fit = mean_squared_error(y_test, y_hat_test_evo)
        #         # print("fit", fit)
        #         # exit()
        #         tbl.append(fit)
        #     print(tbl)
        ...
        clf = LGBMRegressor(n_jobs=6,
                            n_estimators=1000,linear_tree=True,verbose = -100)
