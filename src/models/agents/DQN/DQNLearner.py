...
class DQN(rl_agent.AbstractAgent):
    ...
    def _loss(self, param, param_target, info_states, actions, rewards,
        ...
        max_next_q = jnp.max(target_q_values + jnp.log(legal_actions_mask),
                             axis=-1)
    ...
    def get_update(self):
        ...
        def update(param, param_target, opt_state, info_states, actions,
                   rewards,
                   next_info_states, are_final_steps, legal_actions_mask):
            loss_val, grad_val = self._loss_and_grad(param, param_target,
                                                     info_states,
                                                     actions, rewards,
                                                     next_info_states,
                                                     are_final_steps,
                                                     legal_actions_mask)
            new_param, new_opt_state = self._opt_update_fn(param, opt_state,
                                                           grad_val)
    ...
    def learn(self):
        ...
        self.params_q_network, self._opt_state, loss_val = self._jit_update(
            self.params_q_network, self.params_target_q_network,
            self._opt_state,
