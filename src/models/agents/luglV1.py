...
class DCLF(rl_agent.AbstractAgent):
    ...
    def step(self, time_step, is_evaluation=False):
        ...
        if not is_evaluation:
            ...
            obs = np.array([ord(c) for c in str(child_state).split(":")[-1]])
            ...
            for a in child_state.legal_actions():
                ...
                c_obs = np.array([ord(c) for c in str(grand_child).split(":")[-1]])
                #print(c_obs)
                diff = ((obs != c_obs) * 1)
                #print(c_obs * diff)
                c_obss.extend(c_obs * diff)
            #print("==============")
            ...
            self._prev_info_next_state = tuple(c_obss)
...
class LUGLBLightGBM(DCLF):
    ...
    def train_supervised(self):
        ...
        # x = Dense(100, activation="relu", activity_regularizer="l2")(input)
        ...
        #
        # transformer_full = keras.Model([input], x)
        #
        ...
        # transformer_full.fit(X_train, y_train,
        #                      validation_data=(X_test, y_test), verbose=True, epochs = 4)
        #
        ...
        clf = LGBMRegressor(n_jobs=6,
                            n_estimators=1000,
                            num_leaves=200, linear_tree=True,verbose = -100,categorical_features = list(range(X.shape[1])))
        ...
        clf = LGBMRegressor(n_jobs=6, n_estimators=n_estimators_, num_leaves=200,
                            linear_tree=True,verbose = -100,categorical_features = list(range(X.shape[1])))
