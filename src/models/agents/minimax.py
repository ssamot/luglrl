...
def _alpha_beta(state, depth, alpha, beta, value_function,
  ...
  if state.is_terminal():
    return state.player_return(maximizing_player_id), None, state
  ...
  if depth == 0:
    return value_function(state), None, state
  ...
  if player == maximizing_player_id:
    ...
    for action in state.legal_actions():
      ...
      child_value, _, child_state_d = _alpha_beta(child_state, depth - 1, alpha, beta,
                                   value_function, maximizing_player_id)
      if child_value > value:
        value = child_value
        best_action = action
        best_child = child_state_d

    return value, best_action, best_child
  else:
    value = float("inf")
    for action in state.legal_actions():
      child_state = state.clone()
